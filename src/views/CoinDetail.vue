    ...
    <template v-if="!isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        <div class="flex flex-col items-center">
          ...
          <h1 class="text-5xl">
            {{ asset.name }}
            <small class="sm:mr-2 text-gray-600">{{ asset.symbol }}</small>
          </h1>
        </div>
      ...
      <line-chart
        ...
        :max="max"
        :min="min"
        :data="history.map(i => [i.date, parseFloat(i.priceUsd).toFixed(2)])"
      />
...
export default {
  ...
  computed: {
    ...
    min() {
      return Math.min(
        ...this.history.map(item => Number(item.priceUsd).toFixed(2))
      );
    },
    max() {
      return Math.max(
  ...
  methods: {
    ...
    getWebSite(exchange) {
      ...
      return api
        ...
        .then(resp => {
          // exchange.url = resp.exchangeUrl
          //Esta funcion recibe el "objeto al que queremos adicionar una propiedad"
          //String
          //Valor
